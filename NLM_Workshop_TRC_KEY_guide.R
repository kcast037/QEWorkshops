...
#1 Selfstart for the trc:
trcModel <- function(TA, a, b) {
  y=a * exp(b*TA)
  return(y)
}

#2 Create a function to find initial values for the selfstart function:
...
#3 Selfstart Function
SS.trc <- selfStart(model=trcModel,initial= trc.int)

iv <- getInitial(y ~ SS.trc('TA', "a", "b"), 
                 data = night[which(night$MONTH == j),]) 
iv 

y = nls( NEE ~ (a * exp(b * TA)), night[which(night$MONTH == j),], 
